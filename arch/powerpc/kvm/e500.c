...
{
	preempt_disable();
	vcpu_e500->vcpu.arch.shadow_pid = e500_get_sid(vcpu_e500,
			get_cur_as(&vcpu_e500->vcpu),
			get_cur_pid(&vcpu_e500->vcpu),
			get_cur_pr(&vcpu_e500->vcpu), 1);
	vcpu_e500->vcpu.arch.shadow_pid1 = e500_get_sid(vcpu_e500,
...
unsigned int get_tlb_stid(struct kvm_vcpu *vcpu,
                          struct kvm_book3e_206_tlb_entry *gtlbe)
{
	return e500_get_sid(to_e500(vcpu), get_tlb_ts(gtlbe),
...
{
	struct vcpu_id_table *idt = vcpu_e500->idt;
	unsigned int pr, tid, ts, pid, tsize;
	u32 val, eaddr;
	unsigned long flags;

	ts = get_tlb_ts(gtlbe);
	tid = get_tlb_tid(gtlbe);
	tsize = get_tlb_tsize(gtlbe);
