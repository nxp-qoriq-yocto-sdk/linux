...
static void kvmppc_emul_rfci(struct kvm_vcpu *vcpu)
{
	...
	vcpu->arch.shared->msr = vcpu->arch.csrr1;
}
...
int kvmppc_bookehv_emulate_mtspr(struct kvm_vcpu *vcpu, int sprn, int rs)
{
	...
	case SPRN_DBCR0:
		vcpu->arch.dbg_reg.dbcr0 = spr_val; break;
	case SPRN_DBCR1:
		vcpu->arch.dbg_reg.dbcr1 = spr_val; break;
	case SPRN_DBSR:
...
int kvmppc_bookehv_emulate_mfspr(struct kvm_vcpu *vcpu, int sprn, int rt)
{
	...
	case SPRN_DBCR0:
		kvmppc_set_gpr(vcpu, rt, vcpu->arch.dbg_reg.dbcr0); break;
	case SPRN_DBCR1:
