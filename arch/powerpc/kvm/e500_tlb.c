                                ...
                                int tlbsel, int esel)
{
	...
	kvmppc_core_tlbil_one(vcpu_e500, gtlbe);
}
...
static void clear_tlb1_bitmap(struct kvmppc_vcpu_e500 *vcpu_e500)
{
	if (vcpu_e500->g2h_tlb1_map)
		memset(vcpu_e500->g2h_tlb1_map,
		       sizeof(u64) * vcpu_e500->gtlb_size[1], 0);
	if (vcpu_e500->h2g_tlb1_rmap)
		memset(vcpu_e500->h2g_tlb1_rmap,
		       sizeof(unsigned int) * tlb_host_entries[1], 0);
}
	...
	struct kvm_book3e_206_tlb_entry *stlbe)
{
	...
	stlbe->mas7_3 = ((u64)pfn << PAGE_SHIFT)
		| e500_shadow_mas3_attrib(gtlbe->mas7_3,
				vcpu_e500->vcpu.arch.shared->msr & MSR_PR);
#ifdef CONFIG_KVM_BOOKE_HV
	stlbe->mas8 = MAS8_TGS | vcpu_e500->vcpu.arch.lpid;
#endif
}
	...
	struct kvm_book3e_206_tlb_entry *stlbe)
{
	...
	stlbe->mas7_3 = e500_shadow_mas3_attrib(gtlbe->mas7_3,
				vcpu->arch.shared->msr & MSR_PR);
	stlbe->mas8 = MAS8_TGS | vcpu->arch.lpid | MAS8_VF;
		...
		struct kvm_book3e_206_tlb_entry *stlbe, int esel)
{
	...
	if (vcpu_e500->h2g_tlb1_rmap[victim]) {
		...
		vcpu_e500->g2h_tlb1_map[idx] &= ~((u64)1 << victim);
	}
...
#ifdef CONFIG_KVM_BOOKE_HV
void kvmppc_core_tlbil_one(struct kvmppc_vcpu_e500 *vcpu_e500,
                           struct kvm_book3e_206_tlb_entry *gtlbe)
{
	unsigned int tid, ts;
	u32 val, eaddr, lpid;
...
void kvmppc_core_tlbil_all(struct kvmppc_vcpu_e500 *vcpu_e500)
{
	unsigned long flags;

	local_irq_save(flags);
	mtspr(SPRN_MAS5, MAS5_SGS | (vcpu_e500->vcpu.arch.lpid & 0xFF));
...
int kvmppc_e500_emul_tlbilx(struct kvm_vcpu *vcpu, int rt, int ra, int rb)
{
	...
	if (rt == 0 || rt == 1) {
		...
		for (tlbsel = 0; tlbsel < 2; tlbsel++) {
			for (esel = 0;
			     esel < vcpu_e500->gtlb_size[tlbsel];
			     esel++) {
				tlbe = get_entry(vcpu_e500, tlbsel, esel);
				tid = get_tlb_tid(tlbe);
				if ((rt == 0) || (tid == pid)) {
					inval_gtlbe_on_host(
						vcpu_e500, tlbsel, esel);
		...
		for (tlbsel = 0; tlbsel < 2; tlbsel++) {
			...
			if (esel >= 0) {
				...
				kvmppc_e500_gtlbe_invalidate(vcpu_e500,
					tlbsel, esel);
				break;
			}
		}
			...
			int stlbsel, int sesel)
{
	...
	preempt_disable();
	stid = get_tlb_stid(&vcpu_e500->vcpu, gtlbe);
...
int kvmppc_e500_tlb_init(struct kvmppc_vcpu_e500 *vcpu_e500)
{
	 ...
	 * virtualization case.
	 */
	if (tlb_host_entries[0] == 0 || tlb_host_entries[1] == 0) {
		pr_err("%s: need to know host tlb size\n", __func__);
		return -ENODEV;
	}

	tlb_host_ways[0] = (mfspr(SPRN_TLB0CFG) & TLBnCFG_ASSOC) >>
			   TLBnCFG_ASSOC_SHIFT;
	tlb_host_ways[1] = tlb_host_entries[1];

	if (!is_power_of_2(tlb_host_entries[0]) ||
	    !is_power_of_2(tlb_host_ways[0]) ||
	    tlb_host_entries[0] < tlb_host_ways[0] ||
	    tlb_host_ways[0] == 0) {
		pr_err("%s: bad tlb0 host config: %u entries %u ways\n",
		       __func__, tlb_host_entries[0], tlb_host_ways[0]);
		return -ENODEV;
	}

	tlb_host_sets[0] = tlb_host_entries[0] / tlb_host_ways[0];
	tlb_host_sets[1] = 1;
	...
	vcpu_e500->tlb_refs[0] =
		kzalloc(sizeof(struct tlbe_ref) * tlb_host_entries[0],
			GFP_KERNEL);
	if (!vcpu_e500->tlb_refs[0])
		goto err;

	vcpu_e500->tlb_refs[1] =
		kzalloc(sizeof(struct tlbe_ref) * tlb_host_entries[1],
			GFP_KERNEL);
	...
	vcpu_e500->g2h_tlb1_map =
		kzalloc(sizeof(u64) * vcpu_e500->gtlb_size[1],  GFP_KERNEL);
	if (vcpu_e500->g2h_tlb1_map == NULL)
		goto err;

	vcpu_e500->h2g_tlb1_rmap =
		kzalloc(sizeof(unsigned int) * tlb_host_entries[1], GFP_KERNEL);
	if (vcpu_e500->h2g_tlb1_rmap == NULL)
		goto err;

	/* Init TLB configuration register */
	vcpu_e500->vcpu.arch.tlbcfg[0] = mfspr(SPRN_TLB0CFG) & ~0xfffUL;
	vcpu_e500->vcpu.arch.tlbcfg[0] |= vcpu_e500->gtlb_size[0];
	vcpu_e500->vcpu.arch.tlbcfg[1] = mfspr(SPRN_TLB1CFG) & ~0xfffUL;
	vcpu_e500->vcpu.arch.tlbcfg[1] |= vcpu_e500->gtlb_size[1];
