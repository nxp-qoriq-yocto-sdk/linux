...
{
	/*
	 * When Vin < LSB/2, the val will be 0, then We
	 * designate the Vin is 0 (LSB = Vdd / 1024).
	 */
	if (val == 0)
		return 0;
...
static struct attribute *mcp3021_attribute = &dev_attr_in0_input.attr;
				...
				const struct i2c_device_id *id)
{
	...
	if (client->dev.platform_data) {
		...
		if (data->vdd > MCP3021_VDD_MAX ||
			...
			err = -EINVAL;
			dev_err(&client->dev,
				"The Vdd should be from 2.7V to 5.5v,"
				"%dmv is off design.\n", data->vdd);
			goto exit_free;
		}
	} else
	...
	err = sysfs_create_file(&client->dev.kobj, mcp3021_attribute);
	if (err)
		goto exit_free;
...
exit_remove:
	sysfs_remove_file(&client->dev.kobj, mcp3021_attribute);
exit_free:
	kfree(data);
	return err;
}
